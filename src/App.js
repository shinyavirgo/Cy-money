...
export default function App() {
  ...
  useEffect(() => {
    const setupPWA = () => {
      // 防止網頁縮放，並讓內容填滿手機安全區域 (iPhone 瀏海/動態島)
      ...
      const metaTags = [
        ...
        { name: 'theme-color', content: '#059669' } // 系統主題色
      ];
      ...
      // 產生基本的 Web App Manifest，讓 Android 可以提示安裝至主畫面
      const manifest = {
        name: "專屬記帳系統",
        short_name: "記帳",
        display: "standalone",
  ...
  if (isLoading && !user) {
    return (
      <div className="absolute inset-0 flex items-center justify-center bg-gray-50 text-emerald-600">
        <GlobalStyles />
        載入中...
      </div>
    );
  ...
  if (!user) {
    return (
      <div className="min-h-[100dvh] w-full bg-gray-200 flex flex-col justify-center items-center font-sans p-4 relative">
        <GlobalStyles />
  ...
  return (
    <div className="fixed inset-0 bg-gray-200 flex justify-center font-sans overflow-hidden">
      <GlobalStyles />
      <div className="w-full max-w-md bg-gray-50 relative flex flex-col h-full shadow-2xl">
        ...
        <main className="flex-1 overflow-y-auto p-4 custom-scrollbar pb-32">
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6 pt-4">
              ...
              {/* === 帳號與雲端同步區塊 === */}
              <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
                <h3 className="text-gray-700 font-bold flex items-center gap-2 mb-4"><User size={18} className="text-indigo-500" />帳號與雲端同步</h3>
                ...
                {user && !user.isAnonymous ? (
                   ...
                   <div className="space-y-3">
                     ...
                     <div className="space-y-2">
                       <input type="email" placeholder="輸入 Email" value={authEmail} onChange={e=>setAuthEmail(e.target.value)} className="w-full border border-gray-300 rounded-xl px-3 py-2 text-[16px] outline-none focus:border-indigo-500 transition"/>
                       <input type="password" placeholder="輸入密碼 (至少6位數)" value={authPassword} onChange={e=>setAuthPassword(e.target.value)} className="w-full border border-gray-300 rounded-xl px-3 py-2 text-[16px] outline-none focus:border-indigo-500 transition"/>
                     </div>
